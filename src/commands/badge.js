...
module.exports = {
  ...
  async execute(interaction) {
    await userschema.findOne({ userID: interaction.options.getUser('user').id })
    .then(async result => {
        ...
        if(interaction.options.getString('category') == 'add') {
            ...
            .setColor("#5865f4")
            .setTitle("Badge")
            .setDescription('Please select the badges you want to give the user')
            ...
            .addComponents(
                new MessageSelectMenu()
                  ...
                  },
                  {
                    ...
                    label: 'Featured',
                    description: 'For members who helped the team with building the bot.',
                    value: 'featured'
                  })
                  .setMinValues(1),
              )

            await interaction.reply({ embeds: [badgeEmbed], components: [row], ephemeral: true })
                ...
                listener.on('collect', async (selectMenu) => {
                    if(selectMenu.customId == 'badge-select') {
                        ...
                        await userschema.findOneAndUpdate({ userId: interaction.options.getUser('user').id }, { badges: result.badges }).catch()
            ...
            .setDescription('Please select the badges you want to remove the user')
            ...
            listener.on('collect', async (selectMenu) => {
                if(selectMenu.customId == 'badge-select') {
                    ...
                    await badges.forEach(async badge => {
                        if(result.badges.findIndex(b => b == badge) !== -1) {
                            await result.badges.splice(result.badges.findIndex(b => b == badge), 1);
                        } else return;
                    });
                    ...
                    .setDescription('The badges ' + badges.join(', ') + ' have been removed from the user')
